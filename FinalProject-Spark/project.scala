...
fail.writeStream.outputMode("update").option("truncate",false).format("console").start().awaitTermination(10)

print("Giving the max min and avg age :")
val age="""select max(age),min(age),avg(age) from bankdata"""
val bage = spark.sql(age)
bage.writeStream.outputMode("update").option("truncate",false).format("console").start().awaitTermination(10)
...
ageMatters.writeStream.outputMode("update").option("truncate", false).format("console").start().awaitTermination(10)
...
status.writeStream.outputMode("update").option("truncate", false).format("console").start().awaitTermination(10)
